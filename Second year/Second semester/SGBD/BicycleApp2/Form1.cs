            ...
            try
            {
                ...
                foreach (string column in ColumnNames)
                {
                    if (ColumnTypes[i - 1] == "0")
                    {
                        ...
                        dataAdapter.InsertCommand.Parameters.Add("@column" + i, SqlDbType.VarChar).Value = textBox.Text;                       
                    }
                    if (ColumnTypes[i - 1] == "1")
                    {
                        DateTimePicker dateTimePicker = (DateTimePicker)childPanel.Controls[column];
                        dataAdapter.InsertCommand.Parameters.Add("@column" + i, SqlDbType.VarChar).Value = dateTimePicker.Value.Date;
                    }
                    i++;
                }
        ...
        private void updateButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (string column in ColumnNames)
                {
                    ...
                    {
                        TextBox textBox = (TextBox)childPanel.Controls[column];
                        dataAdapter.UpdateCommand.Parameters.Add("@column" + i, SqlDbType.VarChar).Value = textBox.Text;
                    }
                    if (ColumnTypes[i - 1] == "1")
                    {
                        DateTimePicker dateTimePicker = (DateTimePicker)childPanel.Controls[column];
                        dataAdapter.UpdateCommand.Parameters.Add("@column" + i, SqlDbType.VarChar).Value = dateTimePicker.Value.Date;
                    }
                    i++;
                }
            ...
            foreach (string column in ColumnNames)
            {
                ...
                if (ColumnTypes[i] == "1")
                {
                    ...
                    if (arr[0].Length == 1)
                        ...
                        month = arr[0];
                    year = arr[2];
                    dateTimePicker.Value = new DateTime(int.Parse(year), int.Parse(month), int.Parse(day));
                }
               i++;
            }
        }
