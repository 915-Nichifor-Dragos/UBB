...
void testAddAndSearch(Relation r) {
	...
	ListIterator it = list.first();
	assert(it.valid());
	TElem prev = it.getCurrent();
	it.next();
	while (it.valid()) {
		TElem current = list.getElement(it);
...
void testDeleteSearch(Relation r) {
	...
	assert(it1.valid());
	TElem deleted = NULL_TCOMP;
	try {
		deleted = list.remove(it1);
		assert(list.size() == vMax - vMin);
		TElem newFirst = it1.getCurrent();
	...
	for (int i = 0; i < n; i++) {
		...
		try {
			...
			TElem deleteCurrent = list.remove(it1);
			...
			if (!list.isEmpty()) {
				...
				ListIterator it2 = list.first();
				assert(it2.valid());
				TElem prev = it2.getCurrent();
				while (it2.valid()) {
					TElem current = list.getElement(it2);
					assert(r(prev, current));
